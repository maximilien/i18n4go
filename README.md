...
The command `-extract-strings` pulls strings out of go files.  For the examples below we are running the tool on a copy of the the CloudFoundry CLI cloned in the `./tmp`
...
The general usage for `-create-translations` command is:
