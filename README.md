...
* All dependencies managed via [gvt](https://github.com/FiloSottile/gvt). See [vendor](https://github.com/maximilien/i18n4go/tree/master/vendor) directory on master

### Short `gvt` Guide
* If you ever import a new package `foo/bar`, you can type `gvt fetch foo/bar` to add it to the `vendor` directory.
* To restore dependencies from the `vendor/manifest` file, simply use `gvt restore`.
* If you ever remove a dependency or a link becomes deprecated, use `gvt delete`.
* To update an existing dependency, you can use `gvt update foo`.
* The gvt project [readme](https://github.com/FiloSottile/gvt/blob/master/README.md) is a pretty good resource.
